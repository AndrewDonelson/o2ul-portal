// file: /convex/auth/funcs.providers.ts
// feature: Auth - Provider-specific authentication processing
...
export function normalizeAuthData(rawData: RawAuthData): NormalizedAuthData {
  const provider = rawData.tokenIdentifier.split("|")[0]
    .replace(/^https?:\/\//, '')  // Remove http(s):// prefix
    .split('.')[0];               // Get first subdomain
  ...
  switch (provider) {
    ...
    case "doting-ladybug-105":  // Add this case for your Convex site
      return processEmailAuth(rawData);  // Or create a specific processor if needed
    default:
